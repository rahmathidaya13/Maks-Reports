                ...
                <div class="col-xl-12">
                    ...
                    <div class="card mb-4 overflow-hidden rounded-4">
                        <div class="table-responsive">
                            <tables :headers="header">
                                <tr :id="row.id" v-for="(row, rowIndex) in props.roles.data" :key="rowIndex">
                                    <th class="align-middle">{{ (props.roles.current_page - 1) * props.roles.per_page +
                                        rowIndex
                                        + 1 }}</th>
                                    <td class="align-middle">{{ formatText(row.name) }}</td>
                                    <td class="align-middle">
                                        <ul class="list-unstyled mb-0 d-flex gap-2">
                                            <li v-for="perm in row.permissions" :key="perm.id">
                                                <span class="badge bg-primary">{{ formatText(perm.name) }}</span>
                                            </li>
                                        </ul>
                                    </td>
                                    <td class="align-middle">
                                        <Link :href="route('roles.edit', row.id)" class="btn btn-sm btn-warning">
                                    ...
                                    <tr v-for="(role, index) in props.roles.data" :key="role.id">
                                        <td class="text-center align-middle">
                                            <input type="checkbox" class="form-check-input" :value="role.id"
                                                v-model="selectedRow" />
                                            {{ (props.roles.current_page - 1) * props.roles.per_page + index + 1 }}
                                        </td>
                                        ...
                                        <td class="align-middle">{{ moment(role.created_at).format('LLL') }}</td>
                                        <td class="align-middle">{{ moment(role.updated_at).format('LLL') }}</td>
                                    </tr>
                                </template> -->
                            </tables>
                        </div>
